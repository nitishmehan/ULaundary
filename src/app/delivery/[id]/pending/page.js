...
export default async function PendingDeliveriesPage({ searchParams }) {
  ...
  const pendingDeliveries = await prisma.laundryProcess.findMany({
    where: {
      ...
      ...(fromDate && !toDate && {
        outForDeliveryAt: { gte: new Date(fromDate) }
      }),
      ...(!fromDate && toDate && {
        outForDeliveryAt: { lte: new Date(new Date(toDate).setHours(23, 59, 59, 999)) }
      })
    },
    include: {
      student: {
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-4xl font-bold text-white mb-2">Pending Deliveries</h1>
        <p className="text-gray-400">Deliver laundry to students</p>
      </div>
