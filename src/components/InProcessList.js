...
export default function InProcessList({ requests, staffId }) {
  ...
  const handleReady = async (requestId) => {
    if (!confirm("Mark this laundry as ready for delivery?")) return
      ...
      <div className="mt-6">
        ...
        <div className="space-y-6">
          ...
          <div className="flex gap-4">
            <button
              onClick={() => handleReady(viewingRequest.id)}
              disabled={loading}
              className="flex-1 bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white py-3 px-6 rounded-lg font-semibold disabled:opacity-50"
            >
              {loading ? "Processing..." : "Mark Ready for Delivery"}
            </button>
            <button
              onClick={() => setViewingRequest(null)}
      ...
      <table className="w-full">
        <thead>
          <tr className="border-b border-white/10">
            ...
            <th className="text-center py-4 px-4 text-gray-400 font-medium">Accepted</th>
            ...
            requests.map((request) => (
              <tr key={request.id} className="border-b border-white/5 hover:bg-white/5">
                ...
                <td className="py-4 px-4 text-center text-gray-400 text-sm">
                  {formatDate(request.staffAcknowledgedAt)}
                </td>
                <td className="py-4 px-4 text-center">
                  <button
                    onClick={() => setViewingRequest(request)}
                    className="bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white px-4 py-2 rounded-lg font-semibold text-sm"
                  >
                    View & Process
                  </button>
                </td>
